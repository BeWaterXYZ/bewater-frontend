...
async function getRepoInfo(repoUrl) {
    try {
        ...
        // Get contributors
        const contributors = await githubRequest(`/repos/${owner}/${repo}/contributors?per_page=5`);
        const contributorsInfo = contributors.map(c => ({
            login: c.login,
            avatar_url: c.avatar_url
        }));
        // Get languages
        const languagesData = await githubRequest(`/repos/${owner}/${repo}/languages`);
        ...
        for (const contributor of contributors) {
            try {
                if (!contributor.login)
                    continue;
                const user = await githubRequest(`/users/${contributor.login}`);
                developers.push({
...
async function main() {
    ...
    fs.writeFileSync(path.join(outputDir, 'projects.sql'), generateProjectsSql(projects));
    console.log(`Generated SQL for ${projects.length} projects`);
    fs.writeFileSync(path.join(outputDir, 'developers.sql'), generateDevelopersSql(allDevelopers));
