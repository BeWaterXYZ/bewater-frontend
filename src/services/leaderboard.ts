...
export async function getBuilderboardDeveloper(limit: number, language: string) {
  const { data } = await agentAuthed.get<BuilderboardDeveloper[]>(
    `/billboard/operation/developer-list?limit=${limit}&language=${language}`
  );
  return data;
}

export async function getBuilderboardProject(limit: number) {
  const { data } = await agentAuthed.get<BuilderboardProject[]>(
    `/billboard/operation/project-list?limit=${limit}`
  );
  return data;
}