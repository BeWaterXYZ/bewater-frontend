const sponsors_tier1 = [
  '/sponsors/okx.png',
  '/sponsors/cointelegraph.svg',
  '/sponsors/coinmarketcap.png',
  '/sponsors/cyberport.jpg',
];

const sponsors_tier2 = [
  '/sponsors/aliyun.png',
  '/sponsors/seedao.png',
  '/sponsors/amber.svg',
  '/sponsors/teamz-2023.svg',
  '/sponsors/DIDFollowin.png',
  '/sponsors/amazon.png',
  '/sponsors/KaratDAO4.png',
  '/sponsors/SevenX.png',
  '/sponsors/startfund-logo.png',
  '/sponsors/kucoin.svg',
  '/sponsors/Dorahacks.png',
  '/sponsors/bewater.png',
];

const sponsors_tier3 = [
  '/sponsors/sinohope.png',
  '/sponsors/axlabs.png',
  '/sponsors/coz.png',
  '/sponsors/butterfly_clean.png',
  '/sponsors/abga.png',
  '/sponsors/NeoSPCC.png',
  '/sponsors/next.png',
  '/sponsors/NeoNewsToday-Logo.png',
  '/sponsors/red4sec.png',
  '/sponsors/coinpost.png',
  '/sponsors/builderdao.png',
  '/sponsors/terminal3.svg',
  '/sponsors/HashPalette.svg',
  '/sponsors/soullink.png',
];

export function Sponsors11({ t }: { t: Function }) {
  return (
    <div>
      <h3 className="heading-5 md:heading-3 font-bold mb-16 text-center">
        {t('cryptoArt.t1')}
      </h3>
      <div className="flex flex-col gap-12 items-center">
        <div className="flex flex-col gap-7 items-center">
          <p className="body-1 md:heading-6 font-bold text-white/30 md:text-white/30">
            {t('campaign.t21')}
          </p>
          <div className="flex flex-row flex-wrap gap-0 items-center justify-center">
            {sponsors_tier1.map((src) => (
              <img src={src} key={src} className="h-8 md:h-10 mb-4 mx-4" />
            ))}
          </div>
        </div>
        <div className="flex flex-col gap-7 items-center">
          <p className="body-1 md:heading-6 font-bold text-white/30 md:text-white/30">
            {t('campaign.t22')}
          </p>
          <div className="flex flex-row flex-wrap gap-0 items-center justify-center">
            {sponsors_tier2.map((src) => (
              <img src={src} key={src} className="h-8 md:h-10 mb-4 mx-4" />
            ))}
          </div>
        </div>
        <div className="flex flex-col gap-7 items-center">
          <p className="body-1 md:heading-6 font-bold text-white/30 md:text-white/30">
            {t('campaign.t23')}
          </p>
          <div className="flex flex-row flex-wrap gap-0 items-center justify-center">
            {sponsors_tier3.map((src) => (
              <img src={src} key={src} className="h-8 md:h-10 mb-4 mx-4" />
            ))}
          </div>
        </div>
      </div>
    </div>
  );
}
