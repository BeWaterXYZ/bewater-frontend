...
function Result({ params, challenge, teams, showShortlist }: any) {
  ...
  return (
    <>
          ...
          <div>
            <div className="w-[320px] h-[40px] flex text-[#64748B]">
              ...
              <Link
                ...
                style={{
                  opacity: showShortlist ? "unset" : 0.5,
                  cursor: showShortlist ? "pointer" : "not-allowed",
                  pointerEvents: showShortlist ? "unset" : "none",
                }}
                className="flex-1 border-[1px] border-[#64748B] border-l-0 flex items-center justify-center"
              >
                Shortlist
              </Link>
            </div>
      ...
      {result
        ?.filter((r: ChallengeTrackResult) => r.display !== false)
        .map((w: ChallengeTrackResult) => (
          <>
            {w?.awards.length > 0 && (
              <div className="container" key={w.track}>
                ...
                <p className="rp-text-gradient body-2 text-center my-4 mb-8">
                  {formatMoneyWithCurreny(
                    countTrackAward(w.awards),
                    awardCurrency
                  )}{" "}
