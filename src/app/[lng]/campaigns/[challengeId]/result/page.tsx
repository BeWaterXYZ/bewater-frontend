        ...
        return (
          <div className="container" key={w.track}>
            ...
            <p className="rp-text-gradient body-2 text-center my-4 mb-8">
              {formatMoneyWithCurreny(countTrackAward(w.awards), awardCurrency)} Granted to {w?.awards.length} projects
            </p>
            {w.awards.map((a, index) => (
              <div key={index} className="relative my-8 w-full flex flex-col gap-8">
               {
                a.prize === 1 ? <FirstPrize key={index} lng={lng} award={a} awardCurrency={awardCurrency} /> :
                <OtherPrize key={index} lng={lng} award={a} awardCurrency={awardCurrency} />
              }    
              </div>
            ))}
          </div>
