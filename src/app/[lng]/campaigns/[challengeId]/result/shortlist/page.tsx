...
export default function Page({ params }: any) {
  ...
  const { data, isLoading: shortlistLoading } = useFetchChallengeShortlist(
    challenge?.id ?? ""
  );
  ...
  if (isLoading || teamsLoading || shortlistLoading) {
    return <Loading />;
  }
  if (!showShortlist) {
    return <></>;
    ...
    <div className="container-xl mt-[40px] text-center flex flex-col items-center">
      <div>
        ...
        >
          <Link
            href="."
            className="flex-1 border-[1px] border-[#64748B] flex items-center justify-center"
            style={{
          ...
          data
            ...
            .map((shortlist, i) => (
              <div key={i} className="flex flex-col items-center gap-[40px]">
                {data.length > 1 && (
                  <div className="text-[#FFF] text-3xl/10">
                    {shortlist.name}
                  </div>
                )}
                    ...
                    shortlist.projects.map((project, i) => {
                      ...
                      return (
                        <Card
                          ...
                          headerImage={
                            project.mediaURLs[0] ?? unsplash("project")
                          }
                          avatars={
